...
class GridCellsRNNCell(snt.RNNCore):
  """LSTM core implementation for the grid cell network."""
    ...
    def _build(self, inputs, prev_state):
        ...
        return (ens_outputs, bottleneck, lstm_output, lstm_inputs), tuple(list(next_state))
    ...
    def output_size(self):
        ...
        return tuple([ens.n_cells for ens in self._target_ensembles] + [self._nh_bottleneck, self._nh_lstm, 3])
    ...
    def _build(self, init_conds, vels, training=False):
        ...
        output_seq, final_state = tf.nn.dynamic_rnn(cell=self._core,
                                                    ...
                                                    initial_state=(init_lstm_state,
                                                                   init_lstm_cell))
        ens_targets = output_seq[:-3]
        bottleneck = output_seq[-3]
        lstm_output = output_seq[-2]
        lstm_inputs = output_seq[-1]

        # Return
        return (ens_targets, bottleneck, lstm_output, lstm_inputs, init_lstm_state, init_lstm_cell), final_state
